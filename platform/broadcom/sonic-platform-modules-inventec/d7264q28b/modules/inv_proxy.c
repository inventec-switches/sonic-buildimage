...
read_fan_present(char *attr) {
    ...
    if (sscanf(tmp, "0x%d", &result) != 1)
    {
        return -1;
    }
    // Default setting from kernel hw driver
    // 0x00 indicates that all fans are present
    // 0x11 indicates that the first fan has been removed.  [OOOx]
    // 0x22 indicates that the second fan has been removed. [OOXO]
    // 0x44 indicates that the third fan has been removed.  [OXOO]
    // 0x88 indicates that the fourth fan has been removed. [XOOO]
