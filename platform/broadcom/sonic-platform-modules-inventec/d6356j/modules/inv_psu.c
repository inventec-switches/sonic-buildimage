...
static ssize_t read_mfr_id(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	while (trys <= retrys) {
		...
		temp = i2c_smbus_read_i2c_block_data(client, PMBUS_READ_MFR_ID, I2C_SMBUS_BLOCK_MAX, data->id);
		if (temp < 0) {
			trys++;
			if (trys > retrys)	goto fail;
			continue;
	...
	if(data->id[0]<I2C_SMBUS_BLOCK_MAX) data->id[data->id[0]+1] = 0;
	return sprintf(buf, "%s\n", &data->id[1]);
...
static ssize_t read_mfr_model(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	while (trys <= retrys) {
		...
		temp = i2c_smbus_read_i2c_block_data(client, PMBUS_READ_MFR_MODEL, I2C_SMBUS_BLOCK_MAX, data->model);
		if (temp < 0) {
			trys++;
			if (trys > retrys)	goto fail;
			continue;
	...
	if(data->model[0]<I2C_SMBUS_BLOCK_MAX) data->model[data->model[0]+1] = 0;
	return sprintf(buf, "%s\n", &data->model[1]);
...
static ssize_t read_mfr_revision(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	while (trys <= retrys) {
		...
		temp = i2c_smbus_read_i2c_block_data(client, PMBUS_READ_MFR_REV, I2C_SMBUS_BLOCK_MAX, data->revision);
		if (temp < 0) {
			trys++;
			if (trys > retrys)	goto fail;
			continue;
	...
	if(data->revision[0]<I2C_SMBUS_BLOCK_MAX) data->revision[data->revision[0]+1] = 0;
	return sprintf(buf, "%s\n", &data->revision[1]);
...
static ssize_t read_mfr_location(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	while (trys <= retrys) {
		...
		temp = i2c_smbus_read_i2c_block_data(client, PMBUS_READ_MFR_LOC, I2C_SMBUS_BLOCK_MAX, data->location);
		if (temp < 0) {
			trys++;
			if (trys > retrys)	goto fail;
			continue;
	...
	if(data->location[0]<I2C_SMBUS_BLOCK_MAX) data->location[data->location[0]+1] = 0; 
	return sprintf(buf, "%s\n", &data->location[1]);
...
static ssize_t read_mfr_date(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	while (trys <= retrys) {
		...
		temp = i2c_smbus_read_i2c_block_data(client, PMBUS_READ_MFR_DATE, I2C_SMBUS_BLOCK_MAX, data->date);
		if (temp < 0) {
			trys++;
			if (trys > retrys)	goto fail;
			continue;
	...
	if(data->date[0]<I2C_SMBUS_BLOCK_MAX) data->date[data->date[0]+1] = 0;
	return sprintf(buf, "%s\n", &data->date[1]);
...
static ssize_t read_mfr_sn(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	while (trys <= retrys) {
		...
		temp = i2c_smbus_read_i2c_block_data(client, PMBUS_READ_MFR_SERIAL, I2C_SMBUS_BLOCK_MAX, data->serial);
		if (temp < 0) {
			trys++;
			if (trys > retrys)	goto fail;
			continue;
	...
	if(data->serial[0]<I2C_SMBUS_BLOCK_MAX) data->serial[data->serial[0]+1] = 0;
	return sprintf(buf, "%s\n", &data->serial[1]);
